...
export default function AdminProductEditView({ productId, onNavigate }) {
    ...
    useEffect(() => {
        const loadProduct = async () => {
            try {
                ...
                const imagesMeta = baseProduct ? await getProductImages(productId) : [];
    ...
    const handleUpdate = async (finalData, formImages = []) => {
        try {
            const numericProductId = Number.isFinite(Number(productId))
                ? Number(productId)
                : productId;
    ...
    if (error) {
        ...
        <main className="product-edit-container">
                    ...
                    <button type="button" onClick={() => onNavigate('/admin/products')} className="btn-secondary">
                        Volver a la lista
                    </button>
                </main>
        );
